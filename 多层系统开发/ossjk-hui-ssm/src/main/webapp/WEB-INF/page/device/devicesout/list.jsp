			...
			<div class="panel mt-20">
				<div class="panel-body">
					<div class="clearfix">
						...
						<a href="javascript:;" onclick="insert('报废申请','${pageContext.request.contextPath}/device/devicesout/toInsert.do','800','500')" class="btn btn-primary radius"> 
							<i class="Hui-iconfont">&#xe600;</i> 报废申请
						</a>
							...
							<tbody class="getData-list" data-currPage='1'>
								<c:forEach items="${requestScope.page.records }" var="record">
									<tr class="text-c">
										...
										<td>${record.oname }</td>
										<td>
										<fmt:formatDate value="${record.outdate }" pattern="yyyy-MM-dd"/>
										</td>
										...
										<td>
										<fmt:formatDate value="${record.approvaldate }" pattern="yyyy-MM-dd"/>
										</td>
										<td class="f-14 td-manage">
										<c:if test="${record.status eq 1}">
											<a title="审批" href="javascript:;" onclick="update('审批','${pageContext.request.contextPath}/device/devicesout/toOut.do?&id=${record.id }','1','800','700')" class="btn btn-primary radius">审批</a>
											<a title="编辑" href="javascript:;" onclick="update('编辑','${pageContext.request.contextPath}/device/devicesout/toUpdateA.do?&id=${record.id }','1','800','700')" class="ml-5" style="text-decoration: none"><i class="Hui-iconfont">&#xe6df;</i></a>
											<a title="删除" href="javascript:;" onclick="del(this,'${pageContext.request.contextPath}/device/devicesout/delete.do?&id=${record.id}')" class="ml-5" style="text-decoration: none"><i class="Hui-iconfont">&#xe6e2;</i></a>
										</c:if>
										<c:if test="${record.status eq 2 or record.status eq 3}">
											<a title="编辑" href="javascript:;" onclick="update('编辑','${pageContext.request.contextPath}/device/devicesout/toUpdateB.do?&id=${record.id }','1','800','700')" class="ml-5" style="text-decoration: none"><i class="Hui-iconfont">&#xe6df;</i></a>
										</c:if>
										</td>
									</tr>
								</c:forEach>
